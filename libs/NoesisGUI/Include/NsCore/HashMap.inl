...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<> struct HashKeyInfo<uint64_t>
{
    static bool IsEmpty(uint64_t key) { return key == 0xffffffffffffffff; }
    static void MarkEmpty(uint64_t& key) { key = 0xffffffffffffffff; }
    static uint32_t HashValue(uint64_t key) { return Hash(key); }
    static bool IsEqual(uint64_t lhs, uint64_t rhs) { return lhs == rhs; }
};
...
private:
    ...
    void MarkEmpty() { KeyInfo::MarkEmpty(key); }
    ...
    {
        new (Placement(), &key) KeyT(ForwardArg<Key_>(key_));
        new (Placement(), &value) ValueT(ForwardArg<ValueArgs>(values)...);
    }
...
{
public:
    typedef HashMapImpl<Bucket> Owner;

    Iterator(): mPtr(nullptr), mOwner(nullptr) {}

    typedef typename Bucket::Content Content;
    Content& operator*() const { NS_ASSERT(mPtr); return *mPtr->GetContent(); }
    Content* operator->() const { NS_ASSERT(mPtr); return mPtr->GetContent(); }

    bool operator==(const Iterator& it) const { return mPtr == it.mPtr; }
    bool operator!=(const Iterator& it) const { return mPtr != it.mPtr; }
    Iterator& operator++() { NS_ASSERT(mPtr); ++mPtr; SkipEmpty(); return *this; }

private:
    Iterator(Bucket* ptr, const Owner* owner): mPtr(ptr), mOwner(owner) {}
    ...
    void SkipEmpty()
    {
        ...
        const Bucket* end = mOwner->mBuckets + mOwner->mNumBuckets;

        while (mPtr != end)
        {
            if (!mPtr->IsEmpty()) return;
            ++mPtr;
        }
...
public:
    ...
    ConstIterator(): mPtr(nullptr), mOwner(nullptr) {}
    ConstIterator(const typename Owner::Iterator& it): mPtr(it.mPtr), mOwner(it.mOwner) {}
    ...
    bool operator!=(const ConstIterator& it) const { return mPtr != it.mPtr; }
    ConstIterator& operator++() { NS_ASSERT(mPtr); ++mPtr; SkipEmpty(); return *this; }

private:
    ConstIterator(const Bucket* ptr, const Owner* owner): mPtr(ptr), mOwner(owner) {}
...
inline typename HashMapImpl<Bucket>::ConstIterator HashMapImpl<Bucket>::Begin() const
{
    ...
    ConstIterator it(mBuckets, this);
    it.SkipEmpty();
    return it;
}
...
inline typename HashMapImpl<Bucket>::ConstIterator HashMapImpl<Bucket>::Find(const Key_& key) const
{
    ...
    return ConstIterator(bucket, this);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Bucket> template<class Key_>
inline bool HashMapImpl<Bucket>::Contains(const Key_& key) const
{
    uint32_t hash = Bucket::KeyInfo::HashValue(key);
    return FindBucket(hash, key) != 0;
}
    ...
    Ts&&... args)
{
    ...
    {
        return InsertResult(Iterator(bucket, this), false);
    }

    bucket = InsertIntoBucket(bucket, hash, ForwardArg<Key_>(key), ForwardArg<Ts>(args)...);
    return InsertResult(Iterator(bucket, this), true);
}
...
{
    // For linear probing the recommended load factor is ~50%
...
inline void HashMapImpl<Bucket>::MoveBuckets(Bucket* dst, Bucket* src, uint32_t count)
{
    for (uint32_t i = 0; i < count; i++)
    {
        if (!src->IsEmpty())
        {
            new (Placement(), dst) Bucket(MoveArg(*src));
            src->~Bucket();
        }
        else
        {
...
inline void HashMapImpl<Bucket>::InsertBucketsFrom(Bucket* src, uint32_t count)
{
    for (Bucket* it = src; it != src + count; ++it)
    {
        if (!it->IsEmpty())
        {
            ...
            new (Placement(), bucket) Bucket(MoveArg(*it));
            it->~Bucket();
...
inline void HashMapImpl<Bucket>::Erase(Bucket* bucket)
{
    ...
    for (;;)
    {
        ...
        new (Placement(), bucket) Bucket(MoveArg(*bucket_));
        i = j;
        bucket = bucket_;
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Key, typename Value, unsigned N, typename Bucket>
inline void HashMap<Key, Value, N, Bucket>::ClearShrink()
{
    this->Clear();
    ShrinkToFit();
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename Key, unsigned N, typename Bucket>
inline void HashSet<Key, N, Bucket>::ClearShrink()
{
    this->Clear();
    ShrinkToFit();
}
