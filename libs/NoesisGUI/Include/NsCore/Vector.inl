...
inline T* BaseVector<T>::Emplace(T* pos, Args&&... args)
{
    NS_ASSERT(pos >= Begin());
    NS_ASSERT(pos <= End());
    ...
    if (pos == End())
    {
        EmplaceBack(ForwardArg<Args>(args)...);
        return End() - 1;
    }
    ...
    new (Placement(), pos) T(ForwardArg<Args>(args)...);
    return pos;
}
...
inline void BaseVector<T>::Assign(BaseVector&& v)
{
    ...
    // If v is not using inlined memory steals its buffer
    if (!v.mIsSmall)
    {
        DestroyRange(Begin(), End(), IsPod());
...
inline T& BaseVector<T>::EmplaceBack(Args&&... args)
{
    ...
    new (Placement(), End()) T(ForwardArg<Args>(args)...);
    ForceSize(mSize + 1);
    return Back();
}
...
inline void BaseVector<T>::EraseIf(Predicate predicate)
{
    ...
    if (first != last)
    {
        ...
        typedef IsPod<IsTriviallyCopiable<T>::Result> IsPod;
        DestroyRange(next, last, IsPod());
        ForceSize((uint32_t)(next - Begin()));
    }
}
...
inline void BaseVector<T>::UninitializedMove(T* first, const T* last, T* dest, IsPod<false>)
{
    for (; first != last; ++first, ++dest)
    {
        new (Placement(), dest) T(MoveArg(*first));
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename T, unsigned N>
inline void Vector<T, N>::ClearShrink()
{
    this->Clear();

    if (!this->mIsSmall && this->mCapacity != 0)
    {
        Dealloc(this->Begin());
        this->ResetToSmall(N);
    }
