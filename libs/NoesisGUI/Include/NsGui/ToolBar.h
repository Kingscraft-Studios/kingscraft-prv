...
///
/// ToolBar controls provide an overflow mechanism that places items that do not fit on the ToolBar
/// into an overflow area. Only toolbar elements within a parent ToolBarTray can be moved or resized
/// by the user.
///
/// ToolBar is a HeaderedItemsControl, which means its header and collection of objects can be of
/// any type (such as string, Image, or Panel). For more information, see the HeaderedItemsControl
/// class.
///
...
public:
    ...
    /// Gets or sets a value that indicates when an item should be placed in the overflow panel
    /// instead of in the main panel
    //@{
    ...
    /// Gets or sets a value that indicates whether the ToolBar item is an overflow item
    //@{
