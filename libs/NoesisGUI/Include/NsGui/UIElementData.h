...
public:
    ...
    /// Enumerates the registered events
    typedef void(*EnumEventsCallback)(const RoutedEvent* dp, void* user);
    void EnumEvents(EnumEventsCallback callback, void* user) const;
    ...
    /// Unregisters an event class handler from the owner type
    void UnregisterEventHandler(const RoutedEvent* event);
