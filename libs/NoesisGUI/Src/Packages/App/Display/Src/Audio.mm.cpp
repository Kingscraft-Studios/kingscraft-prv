...
NS_CLANG_WARNING_DISABLE("-Wnested-anon-types")
...
static mal_uint32 OnSendAudioDataToDevice(mal_device*, mal_uint32 frameCount, void* out_)
{
    ...
    while (pendingFrames)
    {
        uint32_t frameCount_ = Min(pendingFrames, (uint32_t)NS_COUNTOF(tmp) / CHANNELS);

        for (uint32_t i = 0; i < MAX_VOICES; i++)
        {
            Voice* voice = voices + i;

            if (voice->active.LoadAcquire())
            {
                ...
                if (frames < frameCount_)
                {
                    ...
                    voice->stream.Reset();
                    voice->active.StoreRelease(false);
                }
            }
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void Audio::Play(const Uri& uri, float volume)
{
    ...
    for (int i = 0; i < MAX_VOICES; i++)
    {
        ...
        if (!voice->active.LoadAcquire())
        {
            Noesis::Ptr<Stream> stream = GUI::LoadXamlResource(uri);
            if (stream != 0)
            {
                NS_LOG_TRACE("[%d] -> Playing '%s'", i, uri.Str());
                voice->volume = volume;
                voice->stream = stream;
                ...
                voice->active.StoreRelease(true);
            }
...
void NoesisApp::Audio::Play(const Noesis::Uri&, float) {}
