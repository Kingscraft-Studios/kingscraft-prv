...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Converts a wide-character argument list (wargv) to a UTF-8 encoded argument list (argv)
    static char** ArgvWToArgv(int argc, wchar_t** wargv)
    {
        uint32_t totalSize = 0;
        for (int i = 0; i < argc; i++)
        {
            totalSize += Noesis::UTF8::UTF16To8((uint16_t*)wargv[i], nullptr, 0);
        }

        void* args = malloc(argc * sizeof(char*) + totalSize);

        uint32_t offset = 0;
        for (int i = 0; i < argc; i++)
        {
            char* str = (char*)args + argc * sizeof(char*) + offset;
            uint32_t numChars = Noesis::UTF8::UTF16To8((uint16_t*)wargv[i], nullptr, 0);
            offset += Noesis::UTF8::UTF16To8((uint16_t*)wargv[i], str, numChars);
            ((char**)args)[i] = str;
        }
    ...
    {
        char** argv = ArgvWToArgv(argc, wargv);
        int ret = NsMain(argc, argv);
        free(argv);
        return ret;
    }
  #else
    ...
    {
        char** argv = ArgvWToArgv(__argc, __wargv);
        int ret = NsMain(__argc, argv);
        free(argv);
        return ret;
    }
  #endif
