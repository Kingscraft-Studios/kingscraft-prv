...
{
    return noesis_hb_style_get_value(font, (hb_style_tag_t)styleTag);
}
//////////////
    ...
    m_axisValues(axisValues)
{
    ...
    noesis_hb_draw_funcs_set_move_to_func(m_drawFuncs,
                                   (void*)rpath_move_to,
                                   nullptr,
                                   nullptr);
    noesis_hb_draw_funcs_set_line_to_func(m_drawFuncs,
                                   (void*)rpath_line_to,
                                   nullptr,
                                   nullptr);
    noesis_hb_draw_funcs_set_quadratic_to_func(m_drawFuncs,
                                        (void*)rpath_quad_to,
                                        nullptr,
                                        nullptr);
    noesis_hb_draw_funcs_set_cubic_to_func(m_drawFuncs,
                                    (void*)rpath_cubic_to,
                                    nullptr,
                                    nullptr);
    noesis_hb_draw_funcs_set_close_path_func(m_drawFuncs,
                                      (void*)rpath_close,
                                      nullptr,
                                      nullptr);
...
{
    auto scriptCount =
        noesis_hb_ot_layout_table_get_script_tags(face, tag, 0, nullptr, nullptr);
    auto scripts = std::vector<hb_tag_t>(scriptCount);
    noesis_hb_ot_layout_table_get_script_tags(face,
                                       tag,
                                       0,
                                       &scriptCount,
                                       scripts.data());
    ...
    {
        auto languageCount = noesis_hb_ot_layout_script_get_language_tags(face,
                                                                   tag,
                                                                   i,
                                                                   0,
                                                                   nullptr,
                                ...
                                unsigned textOffset)
{
    ...
    hb_glyph_position_t* glyph_pos =
        (hb_glyph_position_t*)noesis_hb_buffer_get_glyph_positions(buf, &glyph_count);
    ...
    int textDirectionFlag) const
{
    ...
    while (paragraphStart < text.size())
    {
        ...
        while (runIndex < truns.size())
        {
            ...
            hb_script_t lastScript = (hb_script_t)noesis_hb_unicode_script(ufuncs, point);
                   ...
                   paragraphTextIndex < paragraphLength)
            {
                auto point = text[textIndex];
                hb_script_t script =
                    noesis_hb_unicode_general_category(ufuncs, point) ==
                            HB_UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK
                        ? HB_SCRIPT_INHERITED
                        : (hb_script_t)noesis_hb_unicode_script(ufuncs, point);
...
bool HBFont::hasGlyph(const rive::Unichar missing) const
{
    ...
    return noesis_hb_font_get_nominal_glyph(m_font, missing, &glyph);
}
