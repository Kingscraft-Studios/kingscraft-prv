...
VKRenderContext::VKRenderContext()
{
    ...
    // --- ADD THIS BLOCK ---
#ifdef NS_PLATFORM_LINUX
    mVulkanLib = dlopen("libvulkan.so.1", RTLD_NOW | RTLD_LOCAL);
    if (mVulkanLib == nullptr)
    {
        mVulkanLib = dlopen("libvulkan.so", RTLD_NOW | RTLD_LOCAL);
    }
#endif
...
static FILE* GetPipelineCacheFile(bool write, const VkPhysicalDeviceProperties& props)
{
    ...
    FixedString<256> cachePath;

    if (home != nullptr)
    {
        cachePath.AppendFormat("%s/.cache", home);
        // Note: In a production app, you'd want to mkdir this path
    }
    else
    {
        cachePath.AppendFormat("/tmp");
    }
